...
class Game {
  constructor() {
    this.highScore = 0;
    this.mainAudio = new Audio('sounds/aboveground_bgm.ogg');
    this.dieAudio = new Audio('sounds/mariodie.wav');
    this.reset();
  }
  ...
  draw(ctx, gameLeftTime) {
    ...
    if (this.marios[0].coins > this.highScore) {
      this.highScore = this.marios[0].coins;
    }
    ctx.fillText('High Score: ' + this.highScore,Game.DIM_X - 235, 30);
    let coinImg = new Image();
    coinImg.src = 'sprites/coin.png';
    ctx.drawImage(coinImg, Game.DIM_X - 150, 45);


    ctx.fillText(' x ' + this.marios[0].coins,Game.DIM_X - 100, 80);
